...
1. 修复「王者荣耀 → 游戏助手 → 超级帧率 → 再点原始帧率」后开始的黑闪：关闭插帧
   后厂商的 AP-scale / scale_up 映射表会残留在插帧时的 123Hz 档，SurfaceFlinger
   会把 144fps 的解析（连 default 投票也是）指向 mode 11（1080x2352@123），面板被
   拖到 FHD 组、模块再把 1440p144 拉回来，来回翻转就是黑闪（实测两秒内 21 次
   跨组切换）。
2. 投票过滤补丁不再只服务 `stock_ltps` / `custom_ltpo`：`adfr_off`（完美禁用
   ADFR）同样接入，因为它也是固定刷新率方案，同样会被这张 stale 映射表拖走。
3. 内置 daemon 版本同步至 2.9.35。

## v2.9.34

1. **修复一加 Ace 6（PLQ110）刷入后无法开机**：PLQ110 的内核注入路径一直属于未真机验证状态，而 Ace6 内核是 `CONFIG_PANIC_ON_OOPS=y` + `PANIC_TIMEOUT=-1`（panic 后不会自动重启），KO 里一次坏指针或校验命中就是永久 panic，表现为"刷入后不开机"，只能靠第三方 recovery 删模块自救。现在 PLQ110 **默认不再加载任何注入用内核模块**，回到已真机验证可用的 DTBO + props 方案：安装阶段即使选了 DRM-KO 也会自动改回 DTBO，实验入口只在 WebUI「超频 → 显示应用后端」（有风险确认，且写入实验 opt-in 后才允许注入）。
2. 新增**开机自保护（防砖）**：post-fs-data 记录本次 `boot_id`，service.sh 在系统启动完成并稳定运行一段时间后记录同一个 `boot_id`；若下次开机发现上一次没走完，本次自动跳过全部内核模块注入（付费侧退回 props 方案），并在 module.prop / WebUI 显示提示。此后任何 KO 导致的开机崩溃都只影响一次开机，不需要第三方 recovery 删模块。
...
1. 修复 RMX5200 面板周期性黑闪/亮闪与 dpi 抖动：屏幕静止约 5 秒后，厂商 OTI 会投出
   ...
   `config/adfr_lock/oti_pause_last`，在状态一致时直接短路，于是再也不补发，面板就在
...
5. 修正 PLQ110 超频档位说明：170-199Hz 属于 AP 侧克隆时序，Ace 6 面板
   （AA605_P_7_A0020）原厂时序表最高 165Hz，档位高于上限时面板实测仍为
   165Hz；这与「完美禁用 ADFR」无关，不再提示「需要启用 ADFR」。详见
   `docs/PLQ110_PANEL_RATE_CEILING.md`。
6. WebUI 刷新率页新增「实测当前刷新率」（逐帧回调实测，与 TestUFO 同原理），
   切到高于面板上限的档位后会实测并弹出现场对账，不再只凭日志自证；
   日志脚本新增 `display/panel_rate.txt` 取证段。

## v2.9.31

1. 修复 Ace 6（PLQ110）“完美禁用 ADFR”实际已生效但按钮仍显示“待重启”的问题：props 禁用（persist 属性）跨重启持续有效，生效判定不再要求本开机重新写入标记，只要最后一次应用成功即显示已生效。
...
1. 新增一加 Ace 6（PLQ110）支持：SM8750 平台 6.6 GKI 内核，AA605 面板 1272x2772，DRM-KO 后端提供 170/175/180/185/190/195/199Hz 扩展挡位（挡位与一加 15 一致，尚未真机验证）。
2. 修复付费资源包安装记录残留但运行文件缺失时，WebUI"安装最新付费包"误报"已是最新"导致无法自愈的问题；现在按包内 Manifest 逐文件校验 payload 完整性，缺失时自动重新下载并校验安装。
3. 重新进入模块设置界面后自定义插帧、自定义刷新率等配置的保存与回读路径保持不变，授权面板如实显示付费组件安装状态。
4. 内置 daemon 版本同步至 2.9.25。

## v2.9.24

1. 修复付费资源包已完整安装且授权有效时，WebUI 因缺少旧版 `rmx5200_ltpo_modes.sha256` 旁路文件而拒绝重新启用自制 LTPO 的问题。
2. 自制 LTPO 继续由付费包安装阶段的整包 SHA-256、Manifest 文件清单和逐文件校验提供完整性保证，不再要求包外重复 hash 文件。
3. 保留 DRM-KO、禁用 ADFR 重启生效、实时刷新率设置和 DPI 不变路径。

## v2.9.23

1. 优化 WebUI 首次打开和底栏连续切换：应用列表、授权、插帧状态和应用名按需异步加载，避免随机切换后停顿造成页面卡死。
2. 应用列表恢复显示应用名；刷新率设置实时生效，保留设置页、WebUI 与 `mode.txt` 的统一状态，不修改 DPI。
3. 模块内置从目标设备提取的原生 `curl` 及 `libcrypto`、`libssl`、`libz`、`libc++` 依赖；授权、租约、更新和付费组件下载不再依赖 ROM 是否自带 curl，也不再降级到 wget。
4. 修复网络请求的表单编码、刷新令牌读取和 HTTP 错误正文保留，减少“服务器网络请求失败”误报。
5. 安装阶段限制为 RMX5200、PLK110、PJD110；过滤息屏/切换期间的 `unknown` 前台包名，避免误触发显示策略。
6. 回退未验证的自制 LTPO Hook/daemon 接管实验，保留当前已发布稳定路径和现有 KO；免费 Hook 版本升至 81，确保覆盖设备中的旧测试版本。

## v2.9.22

1. 统一全局/应用刷新率真值为 `mode.txt`：设置页、自定义应用刷新率、搜索结果和游戏助手使用同一份应用回显逻辑，刷新率切换不再触碰 DPI 或分辨率密度。
2. 修复显示系统应用开关后的列表刷新与搜索同步；应用列表后台加载后回到主线程更新，避免切换时卡顿、白屏或崩溃。
3. RMX5200 DRM 高刷路径继续使用已验证的逐档链路时钟与源 timing，不启用固定最大链路时钟实验。
4. 更新免费包内置 Hook、daemon 和 DRM-KO 产物，并同步补强构建、签名和兼容性校验。
