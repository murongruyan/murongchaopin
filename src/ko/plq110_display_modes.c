...
/*
 * PLQ110 Qualcomm DRM-KO backend (OnePlus Ace 6).
 *
...
#define OC_PROFILE_NAME "plq110"
#endif
#ifndef OC_LOG_PREFIX
#define OC_LOG_PREFIX "plq110_display_runtime_modes"
#endif
...
#define OC_PANEL_TOKEN "AA605_P_7_A0020"
#endif
#ifndef OC_EXPECT_60
...
#define OC_MODULE_DESCRIPTION "PLQ110 Qualcomm DRM-KO process_dts-compatible mode injector"
#endif
...
static int __init plq110_display_runtime_modes_init(void)
{
	int rc;

	rc = oc_find_dt_state();
	if (rc) {
		pr_err("plq110_display_runtime_modes: DT discovery failed rc=%d source=%px targets=%u count=%u\n",
			rc, oc_dt.source_node, oc_dt.target_count, oc_dt.count);
		goto fail;
	}
	rc = oc_discover_layout();
	if (rc) {
		pr_err("plq110_display_runtime_modes: runtime layout failed rc=%d display=%px modes=%px modes_off=0x%x panel=%px\n",
			rc, oc_layout.display, oc_layout.modes,
			display_modes_offset, oc_layout.panel);
		goto fail;
	}
	pr_info("plq110_display_runtime_modes: display=%px modes=%px count=%u stride=0x%x source=%u panel=0x%x count_off=0x%x\n",
...
static void __exit plq110_display_runtime_modes_exit(void)
{
	oc_restore_dynamic_modes();
	oc_release_dt();
	applied = false;
...
module_init(plq110_display_runtime_modes_init);
module_exit(plq110_display_runtime_modes_exit);
